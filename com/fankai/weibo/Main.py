        ...
        if card['card_type'] == 9:
            ...
            if blog['created_at'] is not None and ("分钟" in blog['created_at'] or "今天" in blog['created_at']):
                ...
                if not com.fankai.weibo.DatabaseUtil.is_have_blog_id(data["blog_id"],data["user_id"]):
                    print("新纪录")
                    #保存数据库
                    com.fankai.weibo.DatabaseUtil.insert(data["user_id"],data["blog_id"],data["text"],data["source"],data["screen_name"],data["retweeted_status"])
                    #下载微博图片
                    filenames = []
                    if data['pics'] is not None:
                        for i,img_json in enumerate(data['pics'],start=1):
                            ...
                            file_name = str(data["blog_id"]) + "_" + str(data["user_id"]) + str(i) + "." + str(img_url).split('.')[-1:][0]
